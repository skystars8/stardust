//! Ascon-AEAD128 file encryption / decryption CLI.
//!
//! Usage:
//!   ascon E <input> <output>   # encrypt
//!   ascon D <input> <output>   # decrypt
//!
...
const KEY_LEN: usize = 16; // Ascon-AEAD128 key
const NONCE_LEN: usize = 16; // Ascon-AEAD128 nonce
const TAG_LEN: usize = 16; // authentication tag (appended by the crate)
const MIN_CIPHERTEXT_LEN: usize = NONCE_LEN + TAG_LEN;

/// Ascon-AEAD128 encrypt / decrypt tool.
///
/// Requires a 16-byte `key.key` file in the current working directory.
/// Input and output must be simple filenames (no directories).
#[derive(Parser, Debug)]
#[command(
    name = "ascon",
    version,
    about = "Bulletproof Ascon-AEAD128 file encryption/decryption (in-memory, no overwrite)",
    long_about = None
)]
struct Args {
...
fn run() -> Result<()> {
    ...
    if !key_path.is_file() {
        bail!(
            "key file '{}' not found in the current directory (must be exactly 16 bytes)",
            KEY_FILE
        );
    }
    ...
    // Construct the key (AsconAead128Key is a 16-byte array type)
    let key = AsconAead128Key::try_from(key_bytes.as_slice())
...
/// Encrypt: generate a fresh random 16-byte nonce, encrypt, prepend nonce.
fn encrypt_file(cipher: &AsconAead128, plaintext: &[u8]) -> Result<Vec<u8>> {
    let mut nonce_bytes = [0u8; NONCE_LEN];
    getrandom_fill(&mut nonce_bytes)
        .map_err(|e| anyhow::anyhow!("failed to generate random nonce: {e}"))?;
    let nonce = AsconAead128Nonce::try_from(nonce_bytes.as_slice())
        .map_err(|_| anyhow::anyhow!("internal error: nonce length"))?;
...
fn decrypt_file(cipher: &AsconAead128, data: &[u8]) -> Result<Vec<u8>> {
    ...
    let nonce = AsconAead128Nonce::try_from(nonce_bytes)
