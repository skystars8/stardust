...
// Portable CLI (originally written for Linux atomic rename; works on Windows too).
...
#[derive(Parser)]
#[command(author, version, about = "Modern secure file encryption CLI (atomic in-place write)")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn interactive_mode() -> Result<()> {
    println!("🔐 Cipher Interactive Mode (Atomic In-Place)\n");
    loop {
