...
const MAGIC: &[u8; 4] = b"HC21";
const NONCE_SIZE: usize = 32;   // HC-256 uses 256-bit IV
...
fn main() -> Result<()> {
    let arg = env::args().nth(1)
        .ok_or_else(|| anyhow::anyhow!("usage: hc256 <filename>"))?;
...
fn encrypt(path: &Path) -> Result<()> {
    ...
    let mut cipher = Hc256::new(&MASTER_KEY.into(), &nonce.into());
...
fn decrypt(path: &Path) -> Result<()> {
    ...
    let len = data.len();
    const MIN_FOOTER: usize = 4 + 1 + 32 + 8; // MAGIC + extlen + nonce(32) + origlen
