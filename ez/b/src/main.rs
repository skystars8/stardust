...
// Uses AES-256-GCM-SIV with a HARDCODED key
...
const VERSION: u8 = 1;
const NONCE_LEN: usize = 12; // AES-GCM-SIV uses 12-byte (96-bit) nonces
...
fn decrypt_file(input_path: &Path) -> Result<(), String> {
    ...
    let nonce = Nonce::from_slice(&data[11 + name_len..header_len]);
